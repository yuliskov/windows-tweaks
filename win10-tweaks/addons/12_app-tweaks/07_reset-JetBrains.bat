...
set CLION_PROFILE_DIR_TEMPLATE=%UserProfile%\.CLion*
...
echo Closing JetBrains processes...
call %CLOSEPROCESS% idea64.exe
call %CLOSEPROCESS% idea.exe
call %CLOSEPROCESS% clion64.exe
call %CLOSEPROCESS% clion.exe

for /d %%f in ("%IDEA_PROFILE_DIR_TEMPLATE%") do call :IdeaLicenseFix %%f
for /d %%f in ("%CLION_PROFILE_DIR_TEMPLATE%") do call :IdeaLicenseFix %%f
